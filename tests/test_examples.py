...
try:
    optimization_solver_available = pyo.SolverFactory(cobmo.config.config['optimization']['solver_name']).available()
except Exception:
...
if optimization_solver_available:
    ...
    examples_path = os.path.join(cobmo.config.base_path, 'examples')
