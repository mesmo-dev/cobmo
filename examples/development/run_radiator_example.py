...
def main():
    ...
    # Run controller.
    controller = cobmo.optimization_problem.OptimizationProblem(building)
    (
    ...
    thermal_power_plot = (
        output_vector_controller.loc[:, output_vector_controller.columns.str.contains("thermal_power")]
        .stack()
        .rename("thermal_power")
        .reset_index()
    ).hvplot.step(x="time", y="thermal_power", by="output_name", **hvplot_default_options)
    zone_temperature_plot = (
        output_vector_controller.loc[
            :, output_vector_controller.columns.isin(building.zones["zone_name"] + "_temperature")
        ]
        .stack()
        .rename("zone_temperature")
        .reset_index()
    ).hvplot.line(x="time", y="zone_temperature", by="output_name", **hvplot_default_options)
    radiator_water_temperature_plot = (
        state_vector_controller.loc[
            :, state_vector_controller.columns.isin(building.zones["zone_name"] + "_radiator_water_mean_temperature")
        ]
        .stack()
        .rename("radiator_water_temperature")
        .reset_index()
    ).hvplot.line(x="time", y="radiator_water_temperature", by="state_name", **hvplot_default_options)
    radiator_hull_temperature_plot = (
        ...
        .stack()
        .rename("radiator_hull_temperature")
        .reset_index()
    ).hvplot.line(x="time", y="radiator_hull_temperature", by="state_name", **hvplot_default_options)
