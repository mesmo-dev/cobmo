...
class BuildingModel(object):
    ...
    def __init__(
        ...
        building_data = cobmo.data_interface.BuildingData(
            self.scenario_name,
        ...
        self.states = pd.Index(
            pd.concat(
                [
                    ...
                    building_data.zones["zone_name"][building_data.zones["fresh_air_flow_control_type"] == "co2_based"]
                    + "_co2_concentration",
                    # Zone absolute humidity.
                    building_data.zones["zone_name"][building_data.zones["humidity_control_type"] == "humidity_based"]
                    + "_absolute_humidity",
                    # Radiator temperatures.
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_radiator_type"])]
                    ...
                    pd.Series(["storage_state_of_charge"])
        ...
        self.controls = pd.Index(
            pd.concat(
                [
                    ...
                    # Generic HVAC system.
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_generic_type"])]
                    ...
                    # AHU.
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_ahu_type"])]
                    + "_ahu_heat_air_flow",
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_ahu_type"])]
                    + "_ahu_cool_air_flow",
                    # TU.
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_tu_type"])]
                    + "_tu_heat_air_flow",
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_tu_type"])]
                    + "_tu_cool_air_flow",
                    # Vent.
                    building_data.zones["zone_name"][pd.notnull(building_data.zones["hvac_vent_type"])]
                    + "_vent_air_flow",
                    ...
                    # Battery storage.
                    pd.Series(["storage_charge_electric_power", "storage_discharge_electric_power"])
        ...
        self.disturbances = pd.Index(
            pd.concat(
                [
                    ...
                    # Internal gains.
                    pd.Series(building_data.zones["internal_gain_type"].dropna().unique() + "_internal_gain_occupancy"),
                    pd.Series(
                        building_data.zones["internal_gain_type"].dropna().unique() + "_internal_gain_appliances"
                    ),
                    pd.Series(building_data.zones["internal_gain_type"].dropna().unique() + "_warm_water_demand"),
        ...
        self.outputs = pd.Index(
            pd.concat(
                [
                    ...
                    # Power balances.
                    pd.Series(
                        ["electric_power_balance", "thermal_power_cooling_balance", "thermal_power_heating_balance"]
                    ),
                ]
            ),
            name="output_name",
        ...
        def calculate_coefficients_zone():
            ...
            building_data.zones["zone_volume"] = building_data.zones["zone_area"] * building_data.zones["zone_height"]

            # Calculate zone heat capacity (absolute heat capacity) from specific heat capacity.
            building_data.zones["heat_capacity"] = (
                building_data.zones["zone_volume"] * building_data.zones["heat_capacity"]
            )

            # Calculate zone air mass (equivalent to moisture capacity).
            building_data.zones["zone_air_mass"] = (
                building_data.zones["zone_volume"] * building_data.parameters.at["density_air"]
            )
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                ...
                building_data.zones.at[zone_name, "zone_surfaces_wall_area"] = (
                    zone_surfaces["surface_area"] * (1 - zone_surfaces["window_wall_ratio"])
                ).sum()
                building_data.zones.at[zone_name, "zone_surfaces_window_area"] = (
                    zone_surfaces["surface_area"] * zone_surfaces["window_wall_ratio"]
                ).sum()
                building_data.zones.at[zone_name, "zone_surfaces_wall_emissivity"] = zone_surfaces[
                    "emissivity_surface"
                ].mean()
                # TODO: Ignore surfaces with no windows.
                building_data.zones.at[zone_name, "zone_surfaces_window_emissivity"] = zone_surfaces[
                    "emissivity_window"
                ].mean()
        ...
        def calculate_coefficients_surface():
            ...
            building_data.surfaces_adiabatic["heat_capacity"] = (
                building_data.surfaces_adiabatic["surface_area"] * building_data.surfaces_adiabatic["heat_capacity"]
            )
            building_data.surfaces_exterior["heat_capacity"] = (
                building_data.surfaces_exterior["surface_area"] * building_data.surfaces_exterior["heat_capacity"]
            )
            building_data.surfaces_interior["heat_capacity"] = (
                building_data.surfaces_interior["surface_area"] * building_data.surfaces_interior["heat_capacity"]
            )
            ...
            for surface_name, surface_data in building_data.surfaces_exterior.iterrows():
                building_data.surfaces_exterior.at[surface_name, "heat_transfer_coefficient_surface_sky"] = (
                    4.0
                    * building_data.parameters.at["stefan_boltzmann_constant"]
                    * surface_data.at["emissivity_surface"]
                    * surface_data.at["sky_view_factor"]
                    * (
                        building_data.scenarios.at["linearization_exterior_surface_temperature"] / 2.0
                        + building_data.scenarios.at["linearization_sky_temperature"] / 2.0
                        + 273.15
                    )
                    ** 3
                )
                building_data.surfaces_exterior.at[surface_name, "heat_transfer_coefficient_surface_ground"] = (
                    4.0
                    * building_data.parameters.at["stefan_boltzmann_constant"]
                    * surface_data.at["emissivity_surface"]
                    ...
                    * (
                        building_data.scenarios.at["linearization_exterior_surface_temperature"] / 2.0
                        + building_data.scenarios.at["linearization_ambient_air_temperature"] / 2.0
                        + 273.15
                    )
                    ** 3
                )
                if pd.notnull(surface_data.at["window_type"]):
                    building_data.surfaces_exterior.at[surface_name, "heat_transfer_coefficient_window_sky"] = (
                        4.0
                        * building_data.parameters.at["stefan_boltzmann_constant"]
                    ...
                    building_data.surfaces_exterior.at[surface_name, "heat_transfer_coefficient_window_ground"] = (
                        4.0
                        * building_data.parameters.at["stefan_boltzmann_constant"]
                        * surface_data.at["emissivity_window"]
                        * (1.0 - surface_data.at["sky_view_factor"])
                ...
                for zone_name, zone_data in building_data.zones.iterrows():
                    if pd.notnull(zone_data.at["hvac_radiator_type"]):
                        ...
                        thickness_water_layer = (
                            zone_data.at["radiator_water_volume"] / zone_data.at["radiator_panel_area"]
                        )
                        thickness_hull_layer = (
                            # Thickness for hull on one side of the panel.
                            0.5
                            * (zone_data.at["radiator_panel_thickness"] - thickness_water_layer)
                        )
                        ...
                        building_data.zones.at[zone_name, "heat_capacitance_hull"] = (
                            radiator_hull_volume * zone_data.at["radiator_hull_heat_capacity"]
                        )
                        building_data.zones.at[zone_name, "heat_capacitance_water"] = (
                            zone_data.at["radiator_water_volume"] * building_data.parameters.at["water_specific_heat"]
                        )

                        # Calculate fundamental thermal resistances.
                        thermal_resistance_conduction = thickness_hull_layer / (
                            zone_data.at["radiator_hull_conductivity"] * zone_data.at["radiator_panel_area"]
                        )
                        thermal_resistance_convection = 1.0 / (
                            zone_data.at["radiator_convection_coefficient"] * zone_data.at["radiator_panel_area"]
                        )
                        temperature_radiator_surfaces_mean = 0.5 * (
                            0.5
                            * (
                        ...
                        thermal_resistance_radiation_rear = (
                            ...
                            + (
                                # TODO: Use total zone surface area and emissivity?
                                (1.0 - zone_data.at["zone_surfaces_wall_emissivity"])
                                / (zone_data.at["radiator_panel_area"] * zone_data.at["zone_surfaces_wall_emissivity"])
                            )
                        ) / (
                            (
                                4.0
                        ...
                        building_data.zones.at[
                            zone_name, "thermal_resistance_radiator_hull_conduction"
                        ...
                        building_data.zones.at[
                            zone_name, "thermal_resistance_radiator_front_zone_surfaces"
                        ...
                        building_data.zones.at[
                            zone_name, "thermal_resistance_radiator_rear_zone_surfaces"
                        ] = thermal_resistance_star_sum_rear / (0.5 * thermal_resistance_conduction)

                        if (building_data.zones["radiator_panel_number"] == "2").any():
                            thermal_resistance_convection_fin = 1.0 / (
                                thermal_resistance_convection * zone_data.at["radiator_fin_effectiveness"]
                            )
                            ...
                            building_data.zones.at[zone_name, "thermal_resistance_radiator_panel_2_front_zone"] = (
                                0.5 * thermal_resistance_conduction + thermal_resistance_convection_fin
                            )
        ...
        def define_heat_transfer_surfaces_exterior():
            ...
            for surface_name, surface_data in building_data.surfaces_exterior.iterrows():
                if surface_data.at["heat_capacity"] != 0.0:  # Surfaces with non-zero heat capacity
                    ...
                    disturbance_matrix[
                        f"{surface_name}_temperature", "irradiation_" + surface_data.at["direction_name"]
                    ] += (
                        ...
                        * (
                            ...
                            / (2.0 * surface_data.at["heat_transfer_coefficient_conduction_surface"])
                        )
                        ** (-1)
                        / surface_data.at["heat_capacity"]
                    )
                    disturbance_matrix[f"{surface_name}_temperature", "ambient_air_temperature"] += (
                        (
                            building_data.parameters.at["heat_transfer_coefficient_exterior_convection"]
                            + surface_data.at["heat_transfer_coefficient_surface_ground"]
                        )
                    ...
                    disturbance_matrix[f"{surface_name}_temperature", "sky_temperature"] += (
                        surface_data.at["heat_transfer_coefficient_surface_sky"]
                        * surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                    ...
                    state_matrix[f"{surface_name}_temperature", f"{surface_name}_temperature"] += (
                        -1.0
                        * surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                            ...
                            + 1.0 / (2.0 * surface_data.at["heat_transfer_coefficient_conduction_surface"])
                        )
                        ** (-1)
                        / surface_data.at["heat_capacity"]
                    )
                    ...
                    state_matrix[f"{surface_name}_temperature", f"{surface_name}_temperature"] += (
                        ...
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                            1.0 / (building_data.parameters.at["heat_transfer_coefficient_interior_convection"])
                            + 1.0 / (2.0 * surface_data.at["heat_transfer_coefficient_conduction_surface"])
                        )
                        ** (-1)
                        / surface_data.at["heat_capacity"]
                    )
                    state_matrix[f"{surface_name}_temperature", surface_data.at["zone_name"] + "_temperature"] += (
                        surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                            1.0 / (building_data.parameters.at["heat_transfer_coefficient_interior_convection"])
                    ...
                    state_matrix[surface_data.at["zone_name"] + "_temperature", f"{surface_name}_temperature"] += (
                        surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                            1.0 / (building_data.parameters.at["heat_transfer_coefficient_interior_convection"])
                    ...
                    state_matrix[
                        surface_data.at["zone_name"] + "_temperature", surface_data.at["zone_name"] + "_temperature"
                    ] += (
                        -1.0
                        * surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                    ...
                    disturbance_matrix[surface_data.at["zone_name"] + "_temperature", "ambient_air_temperature"] += (
                        (
                            building_data.parameters.at["heat_transfer_coefficient_exterior_convection"]
                            + surface_data.at["heat_transfer_coefficient_surface_ground"]
                        )
                    ...
                    disturbance_matrix[surface_data.at["zone_name"] + "_temperature", "sky_temperature"] += (
                        surface_data.at["heat_transfer_coefficient_surface_sky"]
                        * surface_data.at["surface_area"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
                    ...
                    state_matrix[
                        ...
                        * (
                            ...
                            + 1.0 / building_data.parameters.at["heat_transfer_coefficient_interior_convection"]
                            + 1.0 / (surface_data.at["heat_transfer_coefficient_conduction_surface"])
                        )
                        ** (-1)
                        / building_data.zones.at[surface_data.at["zone_name"], "heat_capacity"]
                    )
                ...
                if surface_data.at["window_wall_ratio"] != 0.0:
                    ...
                    state_matrix[
                        ...
                        * (
                            ...
                            + 1.0 / building_data.parameters.at["heat_transfer_coefficient_interior_convection"]
                            + 1.0 / surface_data.at["heat_transfer_coefficient_conduction_window"]
                        )
                        ** (-1)
                        / building_data.zones.at[surface_data.at["zone_name"], "heat_capacity"]
                    )
            ...
            for surface_name, surface_data in building_data.surfaces_interior.iterrows():
                for zone_name in [surface_data.at["zone_name"], surface_data.at["zone_adjacent_name"]]:
                    if surface_data.at["heat_capacity"] != 0.0:  # Surfaces with non-zero heat capacity
                        ...
                        state_matrix[f"{surface_name}_temperature", f"{zone_name}_temperature"] += (
                            surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                            * (
                                1.0 / building_data.parameters.at["heat_transfer_coefficient_interior_convection"]
                        ...
                        state_matrix[f"{zone_name}_temperature", f"{surface_name}_temperature"] += (
                            surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                            * (
                                1.0 / building_data.parameters.at["heat_transfer_coefficient_interior_convection"]
                        ...
                        state_matrix[f"{zone_name}_temperature", f"{zone_name}_temperature"] += (
                            -1.0
                            * surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                            * (
                        ...
                        zone_adjacent_surface_area = sum(
                            zone_surface_data.at["surface_area"] * (1 - zone_surface_data.at["window_wall_ratio"])
                        ...
                        state_matrix[f"{zone_name}_temperature", zone_adjacent_name + "_temperature"] += (
                            surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                            * (
                                1.0 / building_data.parameters.at["heat_transfer_coefficient_interior_convection"]
        ...
        def define_heat_transfer_infiltration():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                ...
                disturbance_matrix[f"{zone_name}_temperature", "ambient_air_temperature"] += (
                    zone_data.at["infiltration_rate"]
                    / 3600  # 1/h in 1/s.
                    * zone_data.at["zone_volume"]
                    * building_data.parameters.at["heat_capacity_air"]
        ...
        def define_heat_transfer_internal_gains():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["internal_gain_type"]):
                    disturbance_matrix[
                        f"{zone_name}_temperature", zone_data.at["internal_gain_type"] + "_internal_gain_occupancy"
                    ] += (
                    ...
                    disturbance_matrix[
                        f"{zone_name}_temperature", zone_data.at["internal_gain_type"] + "_internal_gain_appliances"
                    ] += (
                        zone_data.at["appliances_heat_gain"] * zone_data.at["zone_area"] / zone_data.at["heat_capacity"]
                    )
        ...
        def define_heat_transfer_hvac_generic():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_generic_type"]):
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_generic_heat_thermal_power"] += (
                        1.0 * zone_data.at["zone_area"] / zone_data.at["heat_capacity"]
                    )
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_generic_cool_thermal_power"] += (
                        -1.0 * zone_data.at["zone_area"] / zone_data.at["heat_capacity"]
                    )
            ...
            if pd.notnull(building_data.zones["hvac_radiator_type"]).any():
                for zone_name, zone_data in building_data.zones.iterrows():
                    if pd.notnull(zone_data.at["hvac_radiator_type"]):
                        ...
                        control_matrix[
                            f"{zone_name}_radiator_water_mean_temperature", f"{zone_name}_radiator_thermal_power"
                        ] += (1.0 * zone_data.at["zone_area"] / zone_data.at["heat_capacitance_water"])
                    ...
                    state_matrix[
                        f"{zone_name}_radiator_hull_rear_temperature", f"{zone_name}_radiator_hull_rear_temperature"
                    ] += (
                        -1.0
                        / (0.5 * zone_data.at["thermal_resistance_radiator_hull_conduction"])
                        / zone_data.at["heat_capacitance_hull"]
                    ...
                    state_matrix[
                        f"{zone_name}_radiator_hull_rear_temperature", f"{zone_name}_radiator_water_mean_temperature"
                    ] += (
                        1.0
                        / (0.5 * zone_data.at["thermal_resistance_radiator_hull_conduction"])
                        / zone_data.at["heat_capacitance_hull"]
                    ...
                    state_matrix[
                        f"{zone_name}_radiator_water_mean_temperature", f"{zone_name}_radiator_hull_rear_temperature"
                    ] += (
                        1.0
                        / (0.5 * zone_data.at["thermal_resistance_radiator_hull_conduction"])
                        / zone_data.at["heat_capacitance_water"]
                    ...
                    state_matrix[
                        f"{zone_name}_radiator_water_mean_temperature", f"{zone_name}_radiator_water_mean_temperature"
                    ] += (
                        -1.0
                        / (0.5 * zone_data.at["thermal_resistance_radiator_hull_conduction"])
                        / zone_data.at["heat_capacitance_water"]
                    ...
                    state_matrix[
                        f"{zone_name}_radiator_hull_front_temperature", f"{zone_name}_radiator_hull_front_temperature"
                    ] += (
                        -1.0
                        / zone_data.at["thermal_resistance_radiator_front_zone"]
                        / zone_data.at["heat_capacitance_hull"]
                    )
                    state_matrix[f"{zone_name}_radiator_hull_front_temperature", f"{zone_name}_temperature"] += (
                        1.0
                        / zone_data.at["thermal_resistance_radiator_front_zone"]
                        / zone_data.at["heat_capacitance_hull"]
                    )
                    state_matrix[f"{zone_name}_temperature", f"{zone_name}_radiator_hull_front_temperature"] += (
                        1.0 / zone_data.at["thermal_resistance_radiator_front_zone"] / zone_data.at["heat_capacity"]
                    )
                    state_matrix[f"{zone_name}_temperature", f"{zone_name}_temperature"] += (
                        -1.0 / zone_data.at["thermal_resistance_radiator_front_zone"] / zone_data.at["heat_capacity"]
                    )
                    ...
                    if zone_data.at["radiator_panel_number"] == "2":
                        ...
                        state_matrix[
                            f"{zone_name}_radiator_panel_1_hull_rear_temperature", f"{zone_name}_temperature"
                        ] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_panel_1_rear_zone"]
                            / zone_data.at["heat_capacitance_hull"]
                        ...
                        state_matrix[
                            f"{zone_name}_temperature", f"{zone_name}_radiator_panel_1_hull_rear_temperature"
                        ] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_panel_1_rear_zone"]
                            / zone_data.at["heat_capacity"]
                        ...
                        state_matrix[
                            f"{zone_name}_radiator_panel_2_hull_front_temperature", f"{zone_name}_temperature"
                        ] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_panel_2_front_zone"]
                            / zone_data.at["heat_capacitance_hull"]
                        ...
                        state_matrix[
                            f"{zone_name}_temperature", f"{zone_name}_radiator_panel_2_hull_front_temperature"
                        ] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_panel_2_front_zone"]
                            / zone_data.at["heat_capacity"]
                    ...
                    state_matrix[f"{zone_name}_radiator_hull_rear_temperature", f"{zone_name}_temperature"] += (
                        1.0
                        / zone_data.at["thermal_resistance_radiator_rear_zone"]
                        / zone_data.at["heat_capacitance_hull"]
                    )
                    state_matrix[f"{zone_name}_temperature", f"{zone_name}_radiator_hull_rear_temperature"] += (
                        1.0 / zone_data.at["thermal_resistance_radiator_rear_zone"] / zone_data.at["heat_capacity"]
                    )
                    state_matrix[f"{zone_name}_temperature", f"{zone_name}_temperature"] += (
                        -1.0 / zone_data.at["thermal_resistance_radiator_rear_zone"] / zone_data.at["heat_capacity"]
                    )
                    ...
                    for surface_name, surface_data in pd.concat(
                        [
                            building_data.surfaces_exterior.loc[
                                building_data.surfaces_exterior["zone_name"].isin([zone_name]), :
                            ],
                        ...
                        # Front.
                        state_matrix[f"{zone_name}_radiator_hull_front_temperature", f"{surface_name}_temperature"] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_front_surfaces"]
                            * surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                        ...
                        state_matrix[f"{surface_name}_temperature", f"{zone_name}_radiator_hull_front_temperature"] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_front_surfaces"]
                            * surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                        ...
                        # Back.
                        state_matrix[f"{zone_name}_radiator_hull_rear_temperature", f"{surface_name}_temperature"] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_rear_surfaces"]
                            * surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
                        ...
                        state_matrix[f"{surface_name}_temperature", f"{zone_name}_radiator_hull_rear_temperature"] += (
                            1.0
                            / zone_data.at["thermal_resistance_radiator_rear_surfaces"]
                            * surface_data.at["surface_area"]
                            * (1 - surface_data.at["window_wall_ratio"])
        ...
        def define_heat_transfer_hvac_ahu():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_ahu_type"]):
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_ahu_heat_air_flow"] += (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        * building_data.parameters.at["heat_capacity_air"]
                    ...
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_ahu_cool_air_flow"] += (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        * building_data.parameters.at["heat_capacity_air"]
        ...
        def define_heat_transfer_hvac_tu():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_tu_type"]):
                    ...
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_tu_cool_air_flow"] += (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        * building_data.parameters.at["heat_capacity_air"]
        ...
        def define_heat_transfer_hvac_vent():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_vent_type"]):
                    control_matrix[f"{zone_name}_temperature", f"{zone_name}_vent_air_flow"] += (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        * building_data.parameters.at["heat_capacity_air"]
        ...
        def define_co2_transfer():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if zone_data.at["fresh_air_flow_control_type"] == "co2_based":
                    state_matrix[f"{zone_name}_co2_concentration", f"{zone_name}_co2_concentration"] += (
                        -1.0
                    ...
                    if pd.notnull(zone_data.at["hvac_ahu_type"]):
                        control_matrix[f"{zone_name}_co2_concentration", f"{zone_name}_ahu_heat_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                        ...
                        control_matrix[f"{zone_name}_co2_concentration", f"{zone_name}_ahu_cool_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                            * building_data.scenarios.at["linearization_zone_air_co2_concentration"]
                    ...
                    if pd.notnull(zone_data.at["hvac_vent_type"]):
                        control_matrix[f"{zone_name}_co2_concentration", f"{zone_name}_vent_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                            * building_data.scenarios.at["linearization_zone_air_co2_concentration"]
                    ...
                    disturbance_matrix[f"{zone_name}_co2_concentration", "constant"] += (
                        -1.0
                        * zone_data.at["infiltration_rate"]
                        / 3600  # 1/h in 1/s.
                        * zone_data.at["zone_volume"]
        ...
        def define_humidity_transfer():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if zone_data.at["humidity_control_type"] == "humidity_based":
                    state_matrix[f"{zone_name}_absolute_humidity", f"{zone_name}_absolute_humidity"] += (
                        -1.0
                        * building_data.scenarios.at["linearization_zone_fresh_air_flow"]
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                    ...
                    if pd.notnull(zone_data.at["hvac_ahu_type"]):
                        control_matrix[f"{zone_name}_absolute_humidity", f"{zone_name}_ahu_heat_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                            * building_data.parameters.at["density_air"]
                        ...
                        control_matrix[f"{zone_name}_absolute_humidity", f"{zone_name}_ahu_cool_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                            * building_data.parameters.at["density_air"]
                    ...
                    if pd.notnull(zone_data.at["hvac_vent_type"]):
                        control_matrix[f"{zone_name}_absolute_humidity", f"{zone_name}_vent_air_flow"] += (
                            -1.0
                            / 1000  # l in m³.
                            * zone_data.at["zone_area"]
                            * building_data.parameters.at["density_air"]
                    ...
                    disturbance_matrix[f"{zone_name}_absolute_humidity", "constant"] += (
                        -1.0
                        * zone_data.at["infiltration_rate"]
                        / 3600  # 1/h in 1/s.
                        * zone_data.at["zone_volume"]
        ...
        def define_storage_state_of_charge():
            ...
            if building_data.scenarios.at["storage_commodity_type"] == "sensible_cooling":
                ...
                # Storage charge.
                control_matrix["storage_state_of_charge", "storage_charge_thermal_power_cooling"] += (
                    100.0  # in %.
                ...
                # Storage discharge.
                control_matrix["storage_state_of_charge", "storage_discharge_thermal_power_cooling"] += (
                    -100.0  # in %.
                    / building_data.scenarios.at["storage_capacity"]
                    / building_data.parameters.at["water_density"]
                    / building_data.parameters.at["water_specific_heat"]
            ...
            if building_data.scenarios.at["storage_commodity_type"] == "sensible_heating":
                ...
                # Storage charge.
                control_matrix["storage_state_of_charge", "storage_charge_thermal_power_heating"] += (
                    100.0  # in %.
                    * building_data.scenarios.at["storage_round_trip_efficiency"]
                    / building_data.scenarios.at["storage_capacity"]
                    / building_data.parameters.at["water_density"]
                ...
                # Storage discharge.
                control_matrix["storage_state_of_charge", "storage_discharge_thermal_power_heating"] += (
                    -100.0  # in %.
                    / building_data.scenarios.at["storage_capacity"]
                    / building_data.parameters.at["water_density"]
                    / building_data.parameters.at["water_specific_heat"]
            ...
            if building_data.scenarios.at["storage_commodity_type"] == "battery":
                ...
                # Storage charge.
                control_matrix["storage_state_of_charge", "storage_charge_electric_power"] += (
                    100.0  # in %.
                    * building_data.scenarios.at["storage_round_trip_efficiency"]
                    / building_data.scenarios.at["storage_capacity"]
                    / 3600
                ...
                # Storage discharge.
                control_matrix["storage_state_of_charge", "storage_discharge_electric_power"] += (
                    -100.0  # in %.
                    / building_data.scenarios.at["storage_capacity"]
                    / 3600
                    / 1000  # kWh in Ws.
            ...
            # Storage losses.
            if pd.notnull(building_data.scenarios.at["storage_type"]):
                state_matrix["storage_state_of_charge", "storage_state_of_charge"] += (
                    -1.0 * building_data.scenarios.at["storage_self_discharge_rate"] / 3600  # %/h in %/s.
                )

        def define_output_zone_temperature():

            for zone_name, zone_data in building_data.zones.iterrows():
                state_output_matrix[f"{zone_name}_temperature", f"{zone_name}_temperature"] = 1.0

        def define_output_zone_co2_concentration():

            for zone_name, zone_data in building_data.zones.iterrows():
                if zone_data.at["fresh_air_flow_control_type"] == "co2_based":
                    state_output_matrix[f"{zone_name}_co2_concentration", f"{zone_name}_co2_concentration"] = 1.0

        def define_output_zone_humidity():

            for zone_name, zone_data in building_data.zones.iterrows():
                if zone_data.at["humidity_control_type"] == "humidity_based":
                    state_output_matrix[f"{zone_name}_absolute_humidity", f"{zone_name}_absolute_humidity"] = 1.0
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["internal_gain_type"]):
                    ...
                    disturbance_output_matrix[
                        "electric_power_balance", zone_data.at["internal_gain_type"] + "_internal_gain_appliances"
                    ] += (1.0 * zone_data.at["appliances_heat_gain"] * zone_data.at["zone_area"] / self.zone_area_total)

                    # Thermal power heating due to warm water demand.
                    if pd.notnull(zone_data.at["warm_water_demand_thermal_power"]):
                        disturbance_output_matrix[
                            "thermal_power_heating_balance", zone_data.at["internal_gain_type"] + "_warm_water_demand"
                        ] += (
                            1.0
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_generic_type"]):
                    ...
                    control_output_matrix[
                        f"{zone_name}_generic_cool_thermal_power", f"{zone_name}_generic_cool_thermal_power"
                    ] = 1.0
                    control_output_matrix[
                        "thermal_power_cooling_balance", f"{zone_name}_generic_cool_thermal_power"
                    ] = (
                        1.0
                    ...
                    # Heating power.
                    control_output_matrix[
                        f"{zone_name}_generic_heat_thermal_power", f"{zone_name}_generic_heat_thermal_power"
                    ] = 1.0
                    control_output_matrix[
                        "thermal_power_heating_balance", f"{zone_name}_generic_heat_thermal_power"
                    ] = (
                        1.0
            ...
            if pd.notnull(building_data.zones["hvac_radiator_type"]).any():
                for zone_name, zone_data in building_data.zones.iterrows():
                    if pd.notnull(zone_data.at["hvac_radiator_type"]):
                        ...
                        control_output_matrix[
                            f"{zone_name}_radiator_thermal_power", f"{zone_name}_radiator_thermal_power"
                        ] = 1.0
                        control_output_matrix[
                            "thermal_power_heating_balance", f"{zone_name}_radiator_thermal_power"
                        ] = (
                            1.0
        ...
        def define_output_hvac_ahu_power():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_ahu_type"]):
                    ...
                    # Obtain parameters.
                    ahu_supply_air_absolute_humidity_setpoint = cobmo.utils.calculate_absolute_humidity_humid_air(
                        zone_data.at["ahu_supply_air_temperature_setpoint"],
                        zone_data.at["ahu_supply_air_relative_humidity_setpoint"],
                    )
                    delta_enthalpy_ahu_recovery = cobmo.utils.calculate_enthalpy_humid_air(
                        building_data.scenarios.at["linearization_zone_air_temperature"],
                        building_data.scenarios.at["linearization_zone_air_absolute_humidity"],
                    ...
                    if (
                        ...
                        delta_enthalpy_ahu_recovery_cooling = max(
                            ...
                            min(
                                0.0,
                                zone_data.at["ahu_return_air_heat_recovery_efficiency"] * delta_enthalpy_ahu_recovery,
                            ),
                        )
                        ...
                        delta_enthalpy_ahu_cooling = cobmo.utils.calculate_dew_point_enthalpy_humid_air(
                            zone_data.at["ahu_supply_air_temperature_setpoint"],
                            zone_data.at["ahu_supply_air_relative_humidity_setpoint"],
                        ) - cobmo.utils.calculate_enthalpy_humid_air(
                            building_data.scenarios.at["linearization_ambient_air_temperature"],
                        ...
                        delta_enthalpy_ahu_heating = cobmo.utils.calculate_enthalpy_humid_air(
                            zone_data.at["ahu_supply_air_temperature_setpoint"],
                            ahu_supply_air_absolute_humidity_setpoint,
                    ...
                    # Air flow.
                    control_output_matrix[f"{zone_name}_ahu_cool_air_flow", f"{zone_name}_ahu_cool_air_flow"] = 1.0
                    control_output_matrix[f"{zone_name}_ahu_heat_air_flow", f"{zone_name}_ahu_heat_air_flow"] = 1.0

                    # Cooling power.
                    control_output_matrix["thermal_power_cooling_balance", f"{zone_name}_ahu_cool_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                        * building_data.parameters.at["density_air"]
                        * (abs(delta_enthalpy_ahu_cooling) - abs(delta_enthalpy_ahu_recovery_cooling))
                        / zone_data.at["ahu_cooling_efficiency"]
                    )
                    control_output_matrix["thermal_power_cooling_balance", f"{zone_name}_ahu_heat_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    # Heating power.
                    control_output_matrix["thermal_power_heating_balance", f"{zone_name}_ahu_cool_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                        * building_data.parameters.at["density_air"]
                        * (abs(delta_enthalpy_ahu_heating) - abs(delta_enthalpy_ahu_recovery_heating))
                        / zone_data.at["ahu_heating_efficiency"]
                    )
                    control_output_matrix["thermal_power_heating_balance", f"{zone_name}_ahu_heat_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    # Fan power.
                    control_output_matrix["electric_power_balance", f"{zone_name}_ahu_cool_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    control_output_matrix["electric_power_balance", f"{zone_name}_ahu_heat_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
        ...
        def define_output_hvac_tu_power():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_tu_type"]):
                    ...
                    # Air flow.
                    control_output_matrix[f"{zone_name}_tu_cool_air_flow", f"{zone_name}_tu_cool_air_flow"] = 1.0
                    control_output_matrix[f"{zone_name}_tu_heat_air_flow", f"{zone_name}_tu_heat_air_flow"] = 1.0

                    # Cooling power.
                    control_output_matrix["thermal_power_cooling_balance", f"{zone_name}_tu_cool_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    # Heating power.
                    control_output_matrix["thermal_power_heating_balance", f"{zone_name}_tu_heat_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    # Fan power.
                    control_output_matrix["electric_power_balance", f"{zone_name}_tu_cool_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
                    ...
                    control_output_matrix["electric_power_balance", f"{zone_name}_tu_heat_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
        ...
        def define_output_hvac_vent_power():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_vent_type"]):
                    ...
                    # Air flow.
                    control_output_matrix[f"{zone_name}_vent_air_flow", f"{zone_name}_vent_air_flow"] = 1.0

                    # Fan power.
                    control_output_matrix["electric_power_balance", f"{zone_name}_vent_air_flow"] = (
                        1.0
                        / 1000  # l in m³.
                        * zone_data.at["zone_area"]
                        / self.zone_area_total
        ...
        def define_output_fresh_air_flow():
            ...
            for zone_name, zone_data in building_data.zones.iterrows():
                if pd.notnull(zone_data.at["hvac_ahu_type"]):
                    control_output_matrix[f"{zone_name}_total_fresh_air_flow", f"{zone_name}_ahu_cool_air_flow"] = 1.0
                    control_output_matrix[f"{zone_name}_total_fresh_air_flow", f"{zone_name}_ahu_heat_air_flow"] = 1.0
                if pd.notnull(zone_data.at["hvac_vent_type"]):
                    control_output_matrix[f"{zone_name}_total_fresh_air_flow", f"{zone_name}_vent_air_flow"] = 1.0
                disturbance_output_matrix[f"{zone_name}_total_fresh_air_flow", "constant"] += (
                    zone_data.at["infiltration_rate"]
                    / 3600  # 1/h in 1/s.
                    * zone_data.at["zone_volume"]
                    * 1000  # m³ in l.
        ...
        def define_output_storage_state_of_charge():
            ...
            if pd.notnull(building_data.scenarios.at["storage_type"]):
                state_output_matrix["storage_state_of_charge", "storage_state_of_charge"] = 1.0

        def define_output_storage_power():

            # Sensible storage cooling.
            if building_data.scenarios.at["storage_commodity_type"] == "sensible_cooling":
                control_output_matrix["thermal_power_cooling_balance", "storage_charge_thermal_power_cooling"] = 1.0
                control_output_matrix[
                    "storage_charge_thermal_power_cooling", "storage_charge_thermal_power_cooling"
                ] = 1.0
                control_output_matrix["thermal_power_cooling_balance", "storage_discharge_thermal_power_cooling"] = -1.0
                control_output_matrix[
                    "storage_discharge_thermal_power_cooling", "storage_discharge_thermal_power_cooling"
                ] = 1.0

            # Sensible storage heating.
            if building_data.scenarios.at["storage_commodity_type"] == "sensible_heating":
                control_output_matrix["thermal_power_heating_balance", "storage_charge_thermal_power_heating"] = 1.0
                control_output_matrix[
                    "storage_charge_thermal_power_heating", "storage_charge_thermal_power_heating"
                ] = 1.0
                control_output_matrix["thermal_power_heating_balance", "storage_discharge_thermal_power_heating"] = -1.0
                control_output_matrix[
                    "storage_discharge_thermal_power_heating", "storage_discharge_thermal_power_heating"
                ] = 1.0

            # Battery storage.
            if building_data.scenarios.at["storage_commodity_type"] == "battery":
                control_output_matrix["electric_power_balance", "storage_charge_electric_power"] = 1.0
                control_output_matrix["storage_charge_electric_power", "storage_charge_electric_power"] = 1.0
                control_output_matrix["electric_power_balance", "storage_discharge_electric_power"] = -1.0
                control_output_matrix["storage_discharge_electric_power", "storage_discharge_electric_power"] = 1.0

        def define_output_plant_power():

            # Cooling.
            control_output_matrix["thermal_power_cooling_balance", "plant_thermal_power_cooling"] = -1.0
            control_output_matrix["plant_thermal_power_cooling", "plant_thermal_power_cooling"] = 1.0
            control_output_matrix["electric_power_balance", "plant_thermal_power_cooling"] = (
                1.0 / building_data.scenarios.at["plant_cooling_efficiency"]
            )

            # Heating.
            control_output_matrix["thermal_power_heating_balance", "plant_thermal_power_heating"] = -1.0
            control_output_matrix["plant_thermal_power_heating", "plant_thermal_power_heating"] = 1.0
            control_output_matrix["electric_power_balance", "plant_thermal_power_heating"] = (
                1.0 / building_data.scenarios.at["plant_heating_efficiency"]
            )

        def define_output_grid_power():

            # Electric.
            control_output_matrix["electric_power_balance", "grid_electric_power"] = -1.0
            control_output_matrix["grid_electric_power", "grid_electric_power"] = 1.0

            # Cooling.
            control_output_matrix["thermal_power_cooling_balance", "grid_thermal_power_cooling"] = -1.0
            control_output_matrix["grid_thermal_power_cooling", "grid_thermal_power_cooling"] = 1.0

            # Heating.
            control_output_matrix["thermal_power_heating_balance", "grid_thermal_power_heating"] = -1.0
            control_output_matrix["grid_thermal_power_heating", "grid_thermal_power_heating"] = 1.0

        def define_output_validation_surface_temperature():

            for surface_name, surface_data in pd.concat(
                [building_data.surfaces_adiabatic, building_data.surfaces_exterior, building_data.surfaces_interior],
                sort=False,
            ).iterrows():
                if surface_data.at["heat_capacity"] != 0.0:  # Surfaces with non-zero heat capacity
                    state_output_matrix[f"{surface_name}_temperature", f"{surface_name}_temperature"] = 1.0
        ...
        def define_output_validation_surfaces_exterior_irradiation_gain_exterior():
            ...
            for surface_name, surface_data in building_data.surfaces_exterior.iterrows():
                if surface_data.at["heat_capacity"] != 0.0:  # Surfaces with non-zero heat capacity
                    disturbance_output_matrix[
                        f"{surface_name}_irradiation_gain_exterior", "irradiation_" + surface_data.at["direction_name"]
        ...
        def define_output_validation_surfaces_exterior_convection_interior():
            ...
            for surface_name, surface_data in building_data.surfaces_exterior.iterrows():
                ...
                if surface_data.at["heat_capacity"] != 0.0:  # Surfaces with non-zero heat capacity
                    ...
                    state_output_matrix[
                        surface_data.at["surface_name"] + "_convection_interior", f"{surface_name}_temperature"
                    ] += (1.0 - surface_data.at["window_wall_ratio"]) * (
                        1.0 / (building_data.parameters.at["heat_transfer_coefficient_interior_convection"])
                        + 1.0 / (2.0 * surface_data.at["heat_transfer_coefficient_conduction_surface"])
                    ) ** (
                    ...
                    disturbance_output_matrix[
                        surface_data.at["surface_name"] + "_convection_interior", "ambient_air_temperature"
                    ] += (
                        (
                            building_data.parameters.at["heat_transfer_coefficient_exterior_convection"]
                            + surface_data.at["heat_transfer_coefficient_surface_ground"]
                    ...
                    disturbance_output_matrix[
                        surface_data.at["surface_name"] + "_convection_interior", "sky_temperature"
                    ] += (
                        surface_data.at["heat_transfer_coefficient_surface_sky"]
                        * (1 - surface_data.at["window_wall_ratio"])
                        * (
        ...
        def define_electricity_price_timeseries():
            ...
            if pd.isnull(building_data.scenarios.at["price_type"]):
                ...
                self.electricity_price_timeseries = pd.DataFrame(
                    [[None, None, 1.0]], columns=["time", "price_type", "price"], index=self.timesteps
                )
        ...
        def define_output_constraint_timeseries():
            ...
            self.output_maximum_timeseries = pd.DataFrame(+1.0 * np.infty, self.timesteps, self.outputs)
            self.output_minimum_timeseries = pd.DataFrame(-1.0 * np.infty, self.timesteps, self.outputs)
            ...
            zones_fixed_fresh_air_flow_index = (
                pd.notnull(building_data.zones["hvac_ahu_type"]) | pd.notnull(building_data.zones["hvac_vent_type"])
            ...
            self.output_minimum_timeseries.loc[:, self.outputs.str.contains("_power")] = 0.0

            # Minimum constraint for flow outputs.
            self.output_minimum_timeseries.loc[:, self.outputs.str.contains("_flow")] = 0.0

            # Minimum / maximum constraint for balance outputs.
            self.output_minimum_timeseries.loc[:, self.outputs.str.contains("_balance")] = 0.0
            self.output_maximum_timeseries.loc[:, self.outputs.str.contains("_balance")] = 0.0

            # Minimum / maximum constraint for zone air temperature.
            self.output_minimum_timeseries.loc[:, building_data.zones["zone_name"] + "_temperature"] = (
                building_data.constraint_timeseries.loc[
                    :, building_data.zones["constraint_type"] + "_minimum_air_temperature"
                ]
            ).values
            self.output_maximum_timeseries.loc[:, building_data.zones["zone_name"] + "_temperature"] = (
                building_data.constraint_timeseries.loc[
            ...
            if pd.notnull(building_data.scenarios.at["storage_type"]):
                self.output_minimum_timeseries.loc[:, "storage_state_of_charge"] = 0.0
                self.output_maximum_timeseries.loc[:, "storage_state_of_charge"] = 100.0  # in %.

            # Electric / thermal grid connections.
            if (not connect_electric_grid) and connect_thermal_grid_cooling:
                self.output_maximum_timeseries.loc[:, "plant_thermal_power_cooling"] = 0.0
            if (not connect_electric_grid) and connect_thermal_grid_heating:
                self.output_maximum_timeseries.loc[:, "plant_thermal_power_heating"] = 0.0
            if not connect_thermal_grid_cooling:
                self.output_maximum_timeseries.loc[:, "grid_thermal_power_cooling"] = 0.0
            if not connect_thermal_grid_heating:
                self.output_maximum_timeseries.loc[:, "grid_thermal_power_heating"] = 0.0
        ...
        def discretize_model():
            ...
            state_matrix_discrete = scipy.linalg.expm(self.state_matrix.values * self.timestep_interval.seconds)
            control_matrix_discrete = (
                np.linalg.matrix_power(self.state_matrix.values, -1)
                .dot(state_matrix_discrete - np.identity(self.state_matrix.shape[0]))
                .dot(self.control_matrix.values)
            )
            disturbance_matrix_discrete = (
                np.linalg.matrix_power(self.state_matrix.values, -1)
                .dot(state_matrix_discrete - np.identity(self.state_matrix.shape[0]))
                .dot(self.disturbance_matrix.values)
            )
    ...
    def simulate(
        self, control_vector: pd.DataFrame, state_vector_initial=None, disturbance_timeseries=None
        ...
        state_vector = pd.DataFrame(np.nan, self.timesteps, self.states)
        state_vector.loc[self.timesteps[0], :] = state_vector_initial
        output_vector = pd.DataFrame(np.nan, self.timesteps, self.outputs)
    ...
    def define_optimization_constraints(
        self, optimization_problem: cobmo.utils.OptimizationProblem, initial_state_is_final_state=False
    ):
        ...
        if initial_state_is_final_state:
            optimization_problem.constraints.append(
                optimization_problem.state_vector[0, :] == optimization_problem.state_vector[-1, :]
            )
        # - Otherwise, set initial state according to the initial state vector.
        else:
            optimization_problem.constraints.append(
                optimization_problem.state_vector[0, :] == self.state_vector_initial.values
            )
        ...
        optimization_problem.constraints.append(
            ...
            == cp.transpose(
                self.state_matrix.values @ cp.transpose(optimization_problem.state_vector[:-1, :])
                + self.control_matrix.values @ cp.transpose(optimization_problem.control_vector[:-1, :])
                + self.disturbance_matrix.values @ np.transpose(self.disturbance_timeseries.iloc[:-1, :].values)
            )
        )
        ...
        optimization_problem.constraints.append(
            ...
            == cp.transpose(
                self.state_output_matrix.values @ cp.transpose(optimization_problem.state_vector)
                + self.control_output_matrix.values @ cp.transpose(optimization_problem.control_vector)
                + self.disturbance_output_matrix.values @ np.transpose(self.disturbance_timeseries.values)
            )
        )

        # Output limits.
        optimization_problem.constraints.append(
            optimization_problem.output_vector >= self.output_minimum_timeseries.values
        )
        optimization_problem.constraints.append(
            optimization_problem.output_vector <= self.output_maximum_timeseries.values
        )

    def define_optimization_objective(self, optimization_problem: cobmo.utils.OptimizationProblem):
    ...
    def get_optimization_results(self, optimization_problem: cobmo.utils.OptimizationProblem) -> dict:

        # Obtain results.
        state_vector = pd.DataFrame(optimization_problem.state_vector.value, index=self.timesteps, columns=self.states)
        control_vector = pd.DataFrame(
            optimization_problem.control_vector.value, index=self.timesteps, columns=self.controls
        )
        output_vector = pd.DataFrame(
            optimization_problem.output_vector.value, index=self.timesteps, columns=self.outputs
        )
